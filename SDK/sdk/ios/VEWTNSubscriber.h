...
/** {zh}
 * @type api
 * @brief VEWTNSubscriber Class
 */
/** {en}
 * @type api
 * @brief VEWTNSubscriber Class
 */
VEWTN_IOS_EXPORT @interface VEWTNSubscriber : NSObject
/** {zh}
 * @type api
 * @brief 开启订阅音视频流。  <br>
 *        调用该方法后，本地用户会收到 onSubscribeStateChanged:subscribeState:reason:{@link #VEWTNEngineDelegate#onSubscribeStateChanged:subscribeState:reason:} 的回调。  <br>
 ...
 *        + -1: 失败。 <br>
 * @notes  <br>
 *       + 一个 VEWTNSubscriber 实例仅可以订阅一路音视频流。<br>
 *       + 订阅音视频流前，请先确保此流已发布。如果未发布，会订阅失败。 <br>
 *       + 订阅前需要先调用 setRemoteView:{@link #VEWTNSubscriber#setRemoteView:} 设置视图。 <br>
 *       + 调用 stopSubscribe{@link #VEWTNSubscriber#stopSubscribe} 可以停止订阅。 <br>
 */
/** {en}
 * @type api
 * @brief Subscribe the remote media streams. <br>
 *        After calling the API, you will receive onSubscribeStateChanged:subscribeState:reason:{@link #VEWTNEngineDelegate#onSubscribeStateChanged:subscribeState:reason:}.
 ...
 *         + < 0: Failure <br>
 * @notes  <br>
 *        + You can only subscribe to one media stream with one VEWTNSubscriber instance. <br>
 *        + Ensure the stream is published before subscribe to the stream. <br>
 *        + Call setRemoteView:{@link #VEWTNSubscriber#setRemoteView:} to set the remote view for the media stream before subscribing. <br>
 *        + Call stopSubscribe{@link #VEWTNSubscriber#stopSubscribe} to stop subscribing.
 */
 ...
 *        + < 0：方法调用失败  <br>
 * @notes <br>
 *        + 调用 startSubscribe:{@link #VEWTNSubscriber#startSubscribe:} 发布音视频流。
 *        + 调用该方法后，会收到 onSubscribeStateChanged:subscribeState:reason:{@link #VEWTNEngineDelegate#onSubscribeStateChanged:subscribeState:reason:} 的回调。
 */
/** {en}
 * @type api
 ...
 *         + < 0: Failure <br>
 * @notes  <br>
 *         + Call startSubscribe:{@link #VEWTNSubscriber#startSubscribe:} to start subscribing. <br>
 *         + Receive onSubscribeStateChanged:subscribeState:reason:{@link #VEWTNEngineDelegate#onSubscribeStateChanged:subscribeState:reason:} after calling this API.
 */
- (int)stopSubscribe;
/** {zh}
 * @type api
