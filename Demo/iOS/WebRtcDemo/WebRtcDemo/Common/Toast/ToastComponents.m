...
- (void)showWithMessage:(NSString *)message view:(UIView *)windowView keep:(BOOL)isKeep isLoading:(BOOL)isLoading icon:(NSString *)icon block:(void (^)(BOOL result))bloc {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        __block ToastView *toastView = [[ToastView alloc] initWithMeeage:message icon:icon isLoading:isLoading isHaveIcon:icon.length?YES:NO];
        [windowView addSubview:toastView];
        [toastView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.centerX.equalTo(windowView.mas_centerX);
...
- (void)showWithMessage:(NSString *)message view:(UIView *)windowView keep:(BOOL)isKeep block:(void (^)(BOOL result))block {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        __block ToastView *toastView = [[ToastView alloc] initWithMeeage:message icon:@"" isLoading:NO isHaveIcon:NO];
        [windowView addSubview:toastView];
        [toastView mas_makeConstraints:^(MASConstraintMaker *make) {
